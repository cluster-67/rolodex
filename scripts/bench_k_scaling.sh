...
color_echo "blue" "Running k-scaling benchmark on dataset=$DATASET"

for k in "${K_VALUES[@]}"; do
    color_echo "yellow" "k=$k: serial"
    "$BUILD_DIR/knn" serial --dataset "$DATASET" -k "$k" 2>&1 | tee "$LOGS_DIR/k${k}-serial.log"
